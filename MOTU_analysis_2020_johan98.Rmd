...
OTUtab <- read.delim("Johan98_filtered.tab")
dim(OTUtab)

# reorient
tt = as.data.frame(t(OTUtab[,15:667]))
tt$sample = names(OTUtab[,15:667])
tt = tt[-1,]
tt$sample = gsub("sample.","",tt$sample)
rownames(tt) = tt$sample
tt = tt[,-568]

# reorient for not filtered
# tt = as.data.frame(t(OTUtab[,15:975]))
# tt$sample = names(OTUtab[,15:975])
# tt = tt[-1,]
# tt$sample = gsub("sample.","",tt$sample)
# rownames(tt) = tt$sample
# tt = tt[,-568]

tt2 = tt %>% mutate_all(funs(as.numeric(as.character(.))))
...
which(rowSums(tt2)<3000)
# only controls since we filtered out

# rarify
RRArare = Rarefy(tt2, depth=3000)$`otu.tab.rff`
#specnumber(RRArare)

# calculate relative reads per sample
RRAtab = as.data.frame(decostand(tt2, method="total"))
...
length(which(colSums(RRAtab_filt)==0))

# Remove those columns
rownames(RRAtab_filt) = rownames(RRAtab)
RRAtab_filt2 = RRAtab_filt[,-(which(colSums(RRAtab_filt)==0))]
dim(RRAtab_filt2)
...
```{r}
tt2$richness = specnumber(tt2)
tt2$total_reads = rowSums(tt2[,(-dim(tt2)[2])])

ggplot(tt2, aes(x=total_reads, y=richness))+
  geom_jitter(width=0.5,height=0.5)

# further investigation of blanks

# identity of each MOTU's most abundant sample
maxes = apply(tt2, 2, which.max)
head(maxes)
maxes = as.data.frame(maxes)
maxes$sampleid = rownames(tt2)[maxes$maxes]

maxes[grep("Blank",maxes$sampleid),]

# calculate using relative freq
maxes = apply(RRAtab, 2, which.max)
head(maxes)
maxes = as.data.frame(maxes)
maxes$sampleid = rownames(RRAtab)[maxes$maxes]

maxes[grep("Blank",maxes$sampleid),]

qplot(rowSums(tt2[1:120,]))+scale_x_continuous(limits=c(0,5000))
...
# save a copy
#write.csv(RRAtab_filt2, "RRAtab_filt2_j98.csv", row.names=T)
...
RRA_rare <- read.csv("RRAtab_filt2_j98.csv")
head(RRA_rare)

names(RRA_rare)[1] = "Sample_ID"
RRA_hosts <- read_excel("SAMPLE_SUMMARY.xlsx",sheet="Sheet1")

head(RRA_hosts)
# Combine MOTU table with host data
# make non RRA table
tt2$Sample_ID = rownames(tt2)
RRA2 = full_join(RRA_hosts, RRA_rare)
tt3 = full_join(RRA_hosts, tt2)
dim(RRA2)

RRA2[which(is.na(RRA2$Species)),]$Sample_ID # only one duplicate sample has no associated info
#RRA2=RRA2[-which(is.na(RRA2$Location)),] # removes positive and neg samples
# note positive and neg
RRA2$Species = as.character(RRA2$Species)
RRA2$Species[which(is.na(RRA2$Species))[1:120]] = "Blank"
RRA2$Species[which(is.na(RRA2$Species))[1]] = "Cow"
RRA2$Species[which(is.na(RRA2$Species))[1:4]] = "Neg"
RRA2$Species[which(is.na(RRA2$Species))] = "Pos"

tt3$Sample_ID[which(is.na(tt3$Species))]
tt3$Species = as.character(tt3$Species)
tt3$Species[which(is.na(tt3$Species))[1:120]] = "Blank"
tt3$Species[which(is.na(tt3$Species))[1]] = "Cow"
tt3$Species[which(is.na(tt3$Species))[1]] = "Elephant"
tt3$Species[which(is.na(tt3$Species))[1]] = "Hartebeest"
tt3$Species[which(is.na(tt3$Species))[1]] = "Plains zebra"
tt3$Species[which(is.na(tt3$Species))[1:4]] = "Neg"
tt3$Species[which(is.na(tt3$Species))] = "Pos"

# Use only the qPCR prevalence
RRA2 = RRA2[which(RRA2$rd_2_qpcr_conducted=="Y" | RRA2$Species %in% c("Blank","Neg","Pos")),]
dim(RRA2)

tt3 = tt3[which(tt3$rd_2_qpcr_conducted=="Y" | tt3$Species %in% c("Blank","Neg","Pos")),]


# remove animals not focused on
RRA2 = RRA2[-which(RRA2$Species %in% c("Bushbuck","Goat","Sheep","Oribi")),]
# All NA values indicate additional zeros
tt3 = tt3[-which(tt3$Species %in% c("Bushbuck","Goat","Sheep","Oribi")),]

names(tt3)
# select columns of interest

RRA2 = RRA2[,c(1:14,16:194)]
tt3 = tt3[,c(1:14,16:582)]
head(RRA2)

# Replace NA values with 0 for samples that were undetected
# This includes things that were not screened
RRA2 = RRA2 %>% mutate_at(vars(MOTU_1:MOTU_557), funs(ifelse(is.na(.),0,.)))
tt3 = tt3 %>% mutate_at(vars(MOTU_1:MOTU_568), funs(ifelse(is.na(.),0,.)))
...
RRA2$richness = specnumber(RRA2[,15:153])
tt3$richness = specnumber(tt3[,16:581])
ggplot(tt3, aes(x=Species, y=richness))+
  geom_boxplot()

# Calculate species level richness
sumR = RRA2 %>% group_by(Location, Species) %>% summarize_at(vars(MOTU_1:MOTU_557),funs(sum(.,na.rm=T)))
richy = specnumber(sumR[,3:141])

# Determine the number of samples examined
numR = RRA2 %>% group_by(Location, Species) %>% summarise(count = n())
...
tt3$Species = as.factor(tt3$Species)
tt3$Species = factor(tt3$Species, levels=levels(reorder(tt3$Species,-tt3$richness)))
levels(tt3$Species)

animal_colors=c("#6CA6CD", "#CD919E", "#EE799F", "#A2CD5A","turquoise", "#7EC0EE", "#CD6090", "#63B8FF",  "blue","#9BCD9B", "#CD8C95", "red","#4F94CD", "#8DB6CD", "#6495ED", "#CDC673", "#BDB76B", "#7AC5CD", "#9370DB", "#6A5ACD", "#7CCD7C", "#87CEFF", "gray70", "gray")

RRA2$Species = as.factor(RRA2$Species)
RRA2$Species = factor(RRA2$Species, levels=levels(reorder(RRA2$Species,-RRA2$richness)))
levels(RRA2$Species)

animal_colors=c("red", "blue", "gray", "#6CA6CD", "#CD919E", "#EE799F", "#A2CD5A", "#7EC0EE", "#CD6090", "#63B8FF", "#9BCD9B", "#CD8C95", "#4F94CD", "#8DB6CD", "#6495ED", "#CDC673", "#BDB76B", "#7AC5CD", "#9370DB", "#6A5ACD", "#7CCD7C", "#87CEFF", "gray70","turquoise")

withzeros = ggplot(tt3, aes(x=reorder(Species,-richness), y=richness))+
...
nozeros = ggplot(tt3[tt3$richness!=0,], aes(x=reorder(Species,-richness,na.rm=T), y=richness))+
  geom_jitter(width=0.01,height=0.3, alpha=0.5, aes(color=Species))+
  geom_boxplot(aes(fill=Species), color="gray50",outlier.shape=NA)+
  guides(fill=F,col=F)+
  theme_bw()+
...
tt3_present = tt3[which(tt3$richness>0),]
MPdata = RRA_present[RRA_present$Location=="Mpala",]
MPdata_tt3 = tt3_present[tt3_present$Location=="Mpala"|tt3_present$Species %in% c("Blank","Pos","Neg"),]
# show distribution
...
MPdata_0$Period[MPdata_0$Period=="NA"] = "None"

richmodMP0.1 = glmmTMB(richness ~ Species+(1|Period), ziformula=~., family="poisson", data= MPdata_0[-which(MPdata_0$Species == "Blank"),], REML=F)

richmodMP0 = glmmTMB(richness ~ Species+(1|Period), ziformula=~1 , data=MPdata_0[-which(MPdata_0$Species == "Blank"),], family="nbinom1", REML=F)
...
testResiduals(simulateResiduals(richmodMP0.1))

summary(richmodMP0.1)
...
# with zeros
ems0 = emmeans(richmodMP0.1, ~Species)
CLD(ems0)
contrast(ems0, adjust="holm")
#MuMIn::r.squaredGLMM(richmodMP0) # not available with zero inflation


# spearman rank correlation of 98% results from my method to this
gtmeth = data.frame(sp = c("PZ","GG","GZ","E", "HZ", "I","O","HA", "D","GI","ELA","HIP","B","CAM","DIK","C","WAR","K","WAT"), rank1 = seq(1:19))
jpmeth = data.frame(sp = c("GG","WAT","O","GZ","PZ","E","I","HA","GI","CAM","DIK","D","HZ","B","K","ELA","HIP","C","WAR"), rank2 = seq(1:19))
compmeth = merge(gtmeth,jpmeth)
cor.test(compmeth$rank1,compmeth$rank2, method="spearman")
...
MP2_expand = left_join(MPdata_0,host_info)
...
ggplot(MP2[-which(MP2$Species=="Hybrid zebra"),], aes(x=Diet, y=richness))+
  geom_boxplot(aes(fill=Diet))+theme_bw(base_size=14)+
...
richmoddy2 = lm(richness ~Diet*log(HomeRange_km2)+log(Body_Mass_kg)+MSW93_Family, data=MP2)

anov = aov(richness ~ Diet, data=MP2)
summary(anov)

exp_rich = lmer(richness ~ Diet + (1|Species), data=MP2_expand)
summary(exp_rich)
ggplot(MP2_expand, aes(x=Diet, y=richness))+
  geom_boxplot()
...
hr = ggplot(MP2[-which(is.na(MP2$MSW93_Species)),], aes(x=log(HomeRange_km2), y=richness))+
  ...
  annotate("text",x=-2.5, y=5.5,label=paste("R^2==0.21"),parse=T, size=5)+
...
# No zeros
LONGdata=RRA_present[RRA_present$Species %in% c("Warthog","Plains zebra","Elephant","Kudu","Buffalo","Impala","Blank","Pos","Neg"),]

ggplot(LONGdata, aes(x=Species, y=MOTU_1))+
  geom_boxplot()
...
d = dredge(richmodLONG0)

d
# Species only is the best model
...
```{r}
specpool(MPdata_0[,15:153],MPdata_0$Species)
...
newMP_sp = newMP %>% group_by(Species) %>% summarize_at(vars(MOTU_1:MOTU_557), funs(sum))
...
speculative = specpool(MPdata_0[,15:153],MPdata_0$Species)
...
MOTU = RRA2 %>% group_by(Species) %>%
  summarize_at(vars(MOTU_1:MOTU_557),funs(sum(.))) %>%
  as.data.frame()%>%
  .[,-1] %>%
  t()%>%
  as.data.frame()
names(MOTU) = levels(as.factor(RRA2$Species))
MOTU = MOTU %>% mutate_at(vars(Pos:NTC), funs(ifelse(.>0,1,0)))
MOTU$Hosts = rowSums(MOTU)
MOTU$MOTU = names(RRA2)[15:153]
MOTU$Hosts = as.numeric(MOTU$Hosts)
...
head(motuinfo)
motu98 = data.frame(cluster = (motuinfo$cluster_sumatra98), name=motuinfo$scientific_name_ok)
...
MPdata_noEle[20,] # strange waterb
MPdata_ok = MPdata_noEle[-20,]

ordMR44 = metaMDS(MPdata_tt3[,15:581],autotransform = T, binary=T, k=2)
plot(ordMR44)
ordat = data.frame(Species = MPdata_tt3$Species, x=ordMR44$points[,1], y=ordMR44$points[,2], sampleID = MPdata_tt3$Sample_ID)
ordatblank = ordat[which(ordat$Species=="Blank"),]
ordatblank$number = as.numeric(gsub("Blank","",ordatblank$sampleID))

levels(ordat$Species)

ggplot(ordat, aes(x=x, y=y))+
  geom_point(aes(col=Species), size=3)+
  scale_color_manual(values=animal_colors)+
  theme_bw(base_size=14)+
  scale_x_continuous(limits=c(-0.6,2))

platecolor = c(rep("red",12),rep("orange",12),rep("yellow",12),rep("yellowgreen",12),rep("green",12),rep("lightblue",12),rep("blue",12), rep("violet",12), rep("magenta",12), rep("black",12))
ordatblank$number = as.factor(as.character(ordatblank$number))
ordatblank$number = factor(ordatblank$number, levels=paste(seq(1,120)))

ggplot(ordatblank, aes(x=x, y=y))+
  geom_point(aes(col=number), size=5)+
  geom_text(aes(label = number), size=2)+
 # scale_color_manual(values=animal_colors)+
  scale_color_manual(values=platecolor)+
  theme_bw(base_size=14)+
  scale_x_continuous(limits=c(-0.6,2))+
  guides(col=F)

which(ordat$Species == "PositiveControl")

MPdata = MPdata[-which(is.na(MPdata$MOTU_1)),]
ordMRC = metaMDS(MPdata[,15:153], k=3, binary=T, distance = "jaccard", trymax=100)

ordMRC = metaMDS(MPdata_ok[,15:189], k=3, binary=T, distance="jaccard", noshare=T, trymax=100, trace=0)
ordMRC$converged

# subset data for Longitudinal species
LONGdata=RRA_present[RRA_present$Species %in% c("Warthog","Plains zebra","Elephant","Kudu","Buffalo","Impala"),c(1:5,15:189)]

# remove any columns that now sum to 0
LONGdata= LONGdata[,-(which(colSums(LONGdata[,-c(1:5)])==0)+5)]
dim(LONGdata)

ordLONG=metaMDS(LONGdata[,6:148], distance="jaccard", binary=T, k=3, noshare=T, trymax=100, trace=0)

#may need to run for more iterations
ordLONG$converged
ordLONG=metaMDS(LONGdata[,6:148], binary=T, k=3, noshare=T, trymax=100,previous.best=ordLONG,trace=0)

# takes some time
ord = metaMDS(RRA_present[15:153], distance="jaccard",binary=T, k=3, noshare=T, trymax=100, trace=0)
ord$converged
ord = metaMDS(RRA_present[15:189], distance="jaccard",binary=T, k=3, noshare=T, trymax=100, trace=0, previous.best = ord)
```
...
factorfit(ordMRC$points,envMRC,1000) #
# Using adonis methods
MRCdist = vegdist(MPdata[,15:153], distance="jaccard")
...
# Using adonis methods
LONGdist = vegdist(LONGdata[,6:148], distance="jaccard")
...
# all data
ALLdist = vegdist(RRA_present[,15:153], distance="jaccard")
envALL = RRA_present[,c(2,3)]
ALLdiv=adonis2(ALLdist ~ Species*Location, data=envALL, permutations=999, method="jaccard")
ALLdiv


#write.csv(RRA_present, "RRA_present.csv",row.names=F)
...
longsmall = LONGdata %>% group_by(Location, Species) %>% summarize_at(vars(MOTU_1:MOTU_557), funs(mean))
...
longdist_small = vegdist(longsmall[,-c(1,2,146)], distance="jaccard")
...
ggplot(alllocs_small, aes(x=Distance, y=jaccard))+
...
MDSdf = data.frame(species=RRA_present$Species, location=RRA_present$Location, x1 = ALLPOINTS[,1], x2 =ALLPOINTS[,2], x3=ALLPOINTS[,3])
...
row.names(mydat)=c("Gazella_granti","Equus_grevyi","Loxodonta_africana","Equus_burchellii","Equus_asinus","Equus_hybrid","Oryx_gazella","Alcelaphus_buselaphus","Aepyceros_melampus","Camelus_dromedarius","Taurotragus_oryx","Hippopotamus_amphibius","Phacochoerus_africanus","Madoqua_kirkii","Kobus_ellipsiprymnus","Giraffa_camelopardalis","Bos_taurus","Tragelaphus_strepsiceros","Syncerus_caffer")
...
mydat = mydat[c(7,8,9,1,15,14,17,19,11,18,16,12,13,10,5,4,2,3),]
...
names(motu_wts)
merge(motu_wts, MOTU[,c(25:27,29)])
motu_wts = unique(left_join(motu_wts, MOTU[,c(25:27,29)]))
...
levels(plotMDSMRC2$colors)= c("red","blue","gray","dodgerblue", "magenta3","green3","maroon1", "maroon3", "maroon", "dodgerblue3",  "deepskyblue3", "dodgerblue4","gold3", "cadetblue1", "greenyellow", "green4", "deepskyblue","lightsteelblue3",  "gold", "darkorchid4", "lightskyblue2", "darkorchid1","gray90","gray70")
...
weirdvec = which((MDSdf$species=="Buffalo" & MDSdf$x2>0.5)|(MDSdf$species == "Hippo" & MDSdf$x1 >-1) )
weirdvec
...
levels(plotMDSLONG$colors)= c("dodgerblue", "magenta3","green3","maroon1", "maroon3", "maroon", "dodgerblue3",  "deepskyblue3", "dodgerblue4","gold3", "cadetblue1", "greenyellow", "green4", "deepskyblue","lightsteelblue3",  "gold", "darkorchid4", "lightskyblue2", "darkorchid1")

ggplot(plotMDSLONG, aes(x=Species, y=x1))+
  geom_point(color=as.character(plotMDSLONG$colors))

levels(droplevels(plotMDSLONG$species))
...
weirdlong = which(plotMDSLONG$Species == "Buffalo" & plotMDSLONG$x1 >0 | plotMDSLONG$Species =="Kudu" & plotMDSLONG$x1 >0)
