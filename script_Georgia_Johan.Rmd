...
OBI1_READS<-t(OBI1_reads)
ClustSeq_Reads<-aggregate(OBI1_READS, by=List_Clust, sum)
...
EXT_CTL=grep("NEG", rownames(OBI_reads)) #maroon these are actually just blanks
POS_CTL=grep("POS", rownames(OBI_reads)) #red
CONTROLS=c(EXT_CTL, EMPTY, POS_CTL)
CONTROLS2=c(EMPTY, EXT_CTL)
...
#chunk10
barplot(rowSums(OBI_reads), col=COL.all, border=BOR.all, xlab="Samples", ylab="Nb reads")

hist(log10(rowSums(OBI_reads[-CONTROLS,])), breaks=40,  col=rgb(0,0,1,0.5), main="", xlab="log10 nb Reads", ylab="Nb samples", xlim=c(0,6))
hist(log10(rowSums(OBI_reads[CONTROLS2,])), breaks=40,  col=rgb(1,0,0,0.5), add=T)
hist(log10(rowSums(OBI_reads[CONTROLS3,])), breaks=40,  col=rgb(1,0,0,0.5), add=T)


########## Attempt new filtering using density
library(reconPlots)
IntersectionFunc<-function(D1, D2){
  D1_True<-density(D1, from=min(D1), to=max(D1))
  tutu<-data.frame(D1_True$x, D1_True$y)
  colnames(tutu)<-c('x', 'y')
  D2_Ctl<-density(D2, from=min(D2), to=max(D2)+1)  
  tutu2<-data.frame(D2_Ctl$x, D2_Ctl$y)
  colnames(tutu2)<-c('x', 'y')
  plot(D1_True)
  lines(D2_Ctl, col='red')
  abline(v=curve_intersect(tutu2, tutu))
  thresh.seqdepth<-curve_intersect(tutu2, tutu)[1]
  return(thresh.seqdepth)
}

D1<-sort(as.vector(log10(rowSums(OBI_reads[-CONTROLS,]))))
D2<-sort(as.vector(log10(rowSums(OBI_reads[CONTROLS2,]))))
(thresh.seqdepth<-IntersectionFunc(D1, D2))
...
thresh.seqdepth=log10(3000) #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ !!!! CUTOFF HERE !!!! log10(3000) 3.485861 LOL these are like exactly the same. Using 3000 then and sticking with what we had
#chunk10
hist(log10(rowSums(OBI_reads[-CONTROLS,])), breaks=40,  col=rgb(0,0,1,0.5), main="", xlab="log10 nb Reads", ylab="Nb samples", xlim=c(0,6))
hist(log10(rowSums(OBI_reads[CONTROLS2,])), breaks=40,  col=rgb(1,0,0,0.5), add=T)
hist(log10(rowSums(OBI_reads[CONTROLS3,])), breaks=40,  col=rgb(1,0,0,0.5), add=T)
...
CONTA=ContaSlayer(OBI_reads, OBI_motus, rownames(OBI_reads)[CONTROLS3])
...
OBI2_motus=OBI_motus[which(rownames(OBI_motus) %in% colnames(OBI2_reads)),]
OBI2_motus$count=colSums(OBI2_reads)
...
OBI3_motus=OBI2_motus[which(rownames(OBI2_motus) %in% colnames(OBI3_reads)),]
...
Data export
```{r}
tmp<-t(OBI3_reads) #should be OBI3 normally but OBI2 for no filter
OBI3_motus$MOTU = rownames(OBI3_motus)
colnames(tmp)=paste("sample:", colnames(tmp), sep="")

write.table(data.frame(OBI3_motus,tmp), 'Johan98_filtered.tab', row.names=T, col.names=T, sep="\t")

####At this stage, you can also convert your table into relative read abundance (RRA) using the 'decostand' function in vegan
Tab_reads_new1<-decostand(OBI3_reads, 'total')
...
Tab_reads_new1[Tab_reads_new1<0.01]<-0
dim(Tab_reads_new1[,(colSums(Tab_reads_new1)!=0)])

dim(OBI3_reads)
OBI3_reads[200:210,1:10] # I think the row names are wrong here
```
