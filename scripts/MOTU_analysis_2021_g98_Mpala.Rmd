...
find_asympt = function(sp){
  buf = RRA2 %>% 
  filter(Species==sp) %>% 
  dplyr::select(MOTU_1:MOTU_557) %>% 
  mutate_at(vars(MOTU_1:MOTU_557), funs(round(.*100, 0))) %>% 
  specaccum()
bm1 = fitspecaccum(buf, "lomolino")
asmp = coef(bm1)[1]
n = RRA2 %>% 
  group_by(Species) %>% 
  summarise(n()) %>% 
...
lapply(netdata$Species[-18],FUN = find_asympt) %>% 
  unlist() %>% 
  matrix(ncol=3, byrow = T) %>% 
  as.data.frame(row.names=netdata$Species[-18]) %>% 
  filter(V3<V1) %>% 
  mutate(pct = V3/V1)
```
...
c_data = c_data[-which(c_data$species %in% c("Camel", "Hippo", "Waterbuck")),]
