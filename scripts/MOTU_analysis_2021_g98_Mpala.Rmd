...
#ggsave("g98_plots/prev_richgraph.png",richgraph, width=10, height=5, units="in", dpi=300, device="png")
```
...
#ggsave("g98_plots/gpplots.png",gpplots, device="png", dpi=300, width=10, height=5, units="in")
```
...
motu98 = data.frame(cluster = (motuinfo$cluster_sumatra98), family_j=motuinfo$family_name_ok, name_j=motuinfo$scientific_name_ok, order_g = motuinfo$Order, family_g = motuinfo$Genus, name_g = motuinfo$unknown)
...
#ggsave("g98_plots/psite_host_distribution2.png", psite_distribution, device="png", width=3, height=12)
```
...
plot(treeNJ2, cex=0.6, show.tip=F, align.tip.label=F, label.offset=0.005)
...
#ggsave("g98_plots/pdplot.png",pdplot, device="png", dpi=300, width=10, height=5, units="in")
...
#ggsave("g98_plots/pdsplots.png",pdspplot, device="png", dpi=300, width=10, height=5, units="in")
```
...
THIS IS WHERE THE CODE HAS BEEN RUN IN ONE GO
...
ordMRC = metaMDS(dplyr::select(RRA_present, MOTU_1:MOTU_557), k=3, binary=T, distance="jaccard",autotransform = F, trymax=100)
ordMRC2 = metaMDS(dplyr::select(RRA_present, MOTU_1:MOTU_557), k=2, binary=T, distance="jaccard",autotransform = F, trymax=100)
ordMRC2 = metaMDS(dplyr::select(RRA_present, MOTU_1:MOTU_557), k=2, binary=T, distance="jaccard",autotransform = F, trymax=100, previous.best = ordMRC2)
...
MRCdist = vegdist(dplyr::select(RRA_present, MOTU_1:MOTU_557), distance="jaccard", binary=T)
...
```{r}
#attach more info
#envMRC2 = left_join(MPdata[,1:14],host_info)

# no missing values allowed
#MRCdist2 = vegdist(MPdata[-which(MPdata$Species%in%c("Hybrid zebra","Camel","Hippo")| MPdata$Sample_ID == "MRC_17_HIP_93") ,15:178], distance="jaccard", binary=T)
#envMRC2 = left_join(MPdata[-which(MPdata$Species%in%c("Hybrid zebra","Camel","Hippo")| MPdata$Sample_ID == "MRC_17_HIP_93"),1:14],host_info)

# To investigate other predictors, aggregate to the species level by averaging
# RRAagg
# no NA values for covariates, so will remove hybrid zebra
MPagg = RRAagg[-which(RRAagg$Species%in%c("Hybrid zebra", "Camel","Hippo")),]

# ordinate the smaller DF
# or RRAagg to include Camela and Hippo
...
ordiR2step(MRCdiv_agg_ufc0,MRCdiv_agg_ufc, trace=F) #41%
...
#ggsave("g98_plots/Species_Level_Ord_uf_2021.png", species_level_ord_uf, device="png")


```

THIS IS WHERE I AM IN THE CODE! ALL RESULTS FROM ABOVE HAVE BEEN ENTERED INTO THE MS
...
psitecent = motu_wts2 %>% group_by(name)%>% summarize_at(vars(MDS1:MDS2), funs(mean(.,na.rm=T)))
...
mdsplot_motu = psitecent %>%
  ...
  annotate(geom="text", x = -6, y=-1.5, label=paste("Stress =", round(ordMRC2$stress,2)), size=5)+
  theme_bw(base_size=18)+guides(fill=F, col=F)
...
avgRRA = RRA3 %>% group_by(Species) %>% summarize_at(vars(MOTU_1:MOTU_557), funs(mean))
...
motu98 = data.frame(OTU = paste("MOTU_",motuinfo$cluster_sumatra98,sep=""), rank=motuinfo$rank_ok, sp=motuinfo$scientific_name_ok, sp2 =motuinfo$unknown)
...
test[which(test$OTU=="MOTU_287"),c(2,4,5,6,7)]$sp_f = "Trichostrongyloidea"
...
test[which(test$OTU=="MOTU_444"),c(2,4,5,6,7)]$sp_f = "Trichostrongylus"
...
test %>% group_by(OTU,sp_f) %>% summarize(n())
head(test)

### old code
test2 = (unique(test[,-4]))

test2 %>% group_by(OTU,sp_f) %>% summarize(n())%>%View()
dim(test2) 
...
# unique values
test2 = test2 %>% dplyr::select(Species, sp_f2, OTU, RRA) %>% unique()
...
nodedata = ggplot(netlong, aes(x=reorder(species,-Value), y=Value))+
  geom_col(aes(fill=species), alpha=0.6)+
    theme_bw(base_size=14)+
  theme(axis.text.x = element_text(angle=90, hjust=1,vjust=0))+
  scale_fill_manual(values=animal_colors[-c(14,18)])+guides(fill=F)+
  facet_wrap(~Measure, scales="free_y")
nodedata
...
head(netdata)
# correct sci name
netdata$sciname = c("Syncerus_caffer","Madoqua_kirkii","Giraffa_camelopardalis","Gazella_granti", "Alcelaphus_buselaphus", "Aepyceros_melampus","Oryx_gazella", "Phacochoerus_africanus" ,  "Bos_taurus","Camelus_dromedarius", "Taurotragus_oryx", "Equus_asinus","Equus_grevyi","Hippopotamus_amphibius", "Equus_burchellii",  "Loxodonta_africana")
...
#ggsave("g98_plots/nodeplot_unipartite.png", nodedata2, device="png")


# hippo zeb

hz= RRA2%>% filter(Species %in% c("Hippo", "Grants gazelle"))
hz_id = which(hz %>% dplyr::select(MOTU_1:MOTU_557) %>% colSums() > 0 ) %>%names()
hz2 = hz %>% dplyr::select(hz_id)
hz2$Species =hz$Species
hz2$ID = hz$Sample_ID

head(hz2)
hz2 = hz2 %>% pivot_longer(MOTU_1:MOTU_444)
...
# do the same for cow-ele
hz= RRA3%>% filter(Species %in% c("Cattle", "Elephant"))
hz_id = which(hz %>% dplyr::select(MOTU_1:MOTU_557) %>% colSums() > 0 ) %>%names()
hz2 = hz %>% dplyr::select(hz_id)
hz2$Species =hz$Species
hz2$ID = hz$Sample_ID
...
head(hz2)
hz2 = hz2 %>% pivot_longer(MOTU_4:MOTU_129)
hz2$bin = ifelse(hz2$value>0,1,0)

ggplot(hz2, aes(x=Species, y=bin))+
  geom_col(aes(fill=ID))+facet_wrap(~name)

#which cow is positive for MOTU_98?
hz2 %>% filter(Species=="Cattle" & name=="MOTU_28" & bin==1) # 0.011 JUST MISSES THRESHOLD

hz2 %>% filter(ID=="MRC_COW_244")%>%
  ggplot(aes(x=name, y=value))+
  geom_col()+
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0))


# MOTU 32 is shared between hippo, eland, gga
...
#ggsave("g98_plots/nodplots_2021b.png", nodplots)



# closeness not defined for disconnected graphs -- use only largest component:
components = components(hostgraph)
...
exts
#ggsave("g98_plots/sp_exts_2021.png", exts, device="png")
...
# deeper dive as per email thread with matt
# plot(D$jackknife$mean ~ D2$jackknife$mean)
# cor(D$jackknife$mean, D2$jackknife$mean)
# 
# length(which(D$jackknife$upper<mean(res^2)))
# length(D$jackknife$upper)
# 116/284
# 
# cophs = data.frame(link=names(D$jackknife$upper), sig=D$jackknife$upper<mean(res^2), res=res^2)
# cophs$species = matrix(unlist(strsplit(cophs$link,"-")),ncol=2, byrow = T)[,1]
# cophs$MOTU = matrix(unlist(strsplit(cophs$link,"-")),ncol=2, byrow = T)[,2]
# 
# library(ggmosaic)
# ggplot(cophs, aes(x=reorder(species,-res), y=res))+
#   geom_boxplot()+
#   theme(axis.text.x=element_text(angle=90))
# 
# mosaicplot(species~sig, data=cophs, col=c("gray20","orangered"), las=2)
# 
# 
# head(motu98)
# head(ttlab)
# ttlab[which(ttlab$scientific_name_ok=="Cloacinidae"),]
# 
# 
# 
# # split bovids off an analyze them separately
# 
# tree_bovids = drop.tip(tree, c("Loxodonta_africana","Equus_asinus","Equus_grevyi", "Equus_burchellii", "Hippopotamus_amphibius", "Phacochoerus_africanus", "Camelus_dromedarius", "Giraffa_camelopardalis"))
# # prune host tree
# 
# htree_bovid = cophenetic(tree_bovids)
# # need to rename treeNJ tips
# 
# avgRRA$Species
# gllink = left_join(avgRRA[-which(avgRRA$Species %in% c("Elephant","Donkey","Grevys zebra", "Plains zebra", "Hippo", "Warthog", "Camel", "Giraffe")), ], host_info[,c(1,6)])
# gllink2 = as.data.frame(gllink[-which(is.na(gllink$MSW93_Binomial)==T),])
# rownames(gllink2)=gllink$MSW93_Binomial[-which(is.na(gllink$MSW93_Binomial)==T)]
# gllink2 = gllink2[,-c(1,166)]
# 
# D_bovid = prepare_paco_data(H=htree_bovid, P=ptree, HP=gllink2)
# D_bovid = add_pcoord(D_bovid, correction="cailliez")
# 
# D_bovid = PACo(D_bovid, nperm=1000, seed=12, method="r0", symmetric=F)
# 
# D_bovid = paco_links(D_bovid)
# 
# D_bovid$gof
# 
# 
# # non bovid
# 
# tree_non = drop.tip(tree, c("Oryx_gazella","Alcelaphus_buselaphus","Aepyceros_melampus", "Gazella_granti", "Kobus_ellipsiprymnus", "Madoqua_kirkii", "Bos_taurus", "Syncerus_caffer", "Taurotragus_oryx", "Tragelaphus_strepsiceros"))
# # prune host tree
# 
# htree_non = cophenetic(tree_non)
# # need to rename treeNJ tips
# 
# avgRRA$Species
# gllink = left_join(avgRRA[which(avgRRA$Species %in% c("Elephant","Donkey","Grevys zebra", "Plains zebra", "Hippo", "Warthog", "Camel", "Giraffe")), ], host_info[,c(1,6)])
# #gllink2 = as.data.frame(gllink[-which(is.na(gllink$MSW93_Binomial)==T),])
# gllink=as.data.frame(gllink)
# rownames(gllink)=gllink$MSW93_Binomial
# gllink2 = gllink[,-c(1,166)]
# 
# D_non = prepare_paco_data(H=htree_non, P=ptree, HP=gllink2)
# D_non = add_pcoord(D_non, correction="cailliez")
# D_non = PACo(D_non, nperm=1000, seed=12, method="r0", symmetric=F)
# D_non = paco_links(D_non)
# D_non$gof
# 
# 
# res = residuals_paco(D_bovid$proc)
# mean(res^2)
# length(D_bovid$jackknife$mean)
# 
# Dres = data.frame(res=res, connection=names(D_bovid$jackknife$mean))
# Dres =Dres[order(Dres$res),]
# Dres$sp = rep("",155) #129 for non
# Dres$MOTU = rep("",155)
# for(i in 1:length(Dres$res)){
#   Dres$sp[i]=strsplit(Dres$connection, split="-")[[i]][1]
#   Dres$MOTU[i]=strsplit(Dres$connection, split="-")[[i]][2]
...
# 
# 
# cophs = data.frame(link=names(D_bovid$jackknife$upper), sig=D_bovid$jackknife$upper<mean(res^2), res=res^2)
# cophs$species = matrix(unlist(strsplit(cophs$link,"-")),ncol=2, byrow = T)[,1]
# cophs$MOTU = matrix(unlist(strsplit(cophs$link,"-")),ncol=2, byrow = T)[,2]
# 
# library(ggmosaic)
