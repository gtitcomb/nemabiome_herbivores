...
OTUtab <- read.csv("allMOTUs_sumatra98_GTmethod2020.csv")
dim(OTUtab)
...
RRAtab <- OTUtabsmall %>% mutate_at(vars(MOTU_1:MOTU_568), funs(./total_reads))
...
length(which(colSums(RRAtab_filt[,2:569])==0))

# Remove those columns
RRAtab_filt2 <- RRAtab_filt[,-(which(colSums(RRAtab_filt[,2:569])==0)+1)]
dim(RRAtab_filt2)
...
OTUtabsmall <- OTUtab[-which(rowSums(OTUtab[,-c(1,570,571)])<100),]
...
# Currently we have 175 MOTUs in 663 samples
mean(RRAtab_filt2$total_reads)
median(RRAtab_filt2$total_reads)
min(RRAtab_filt2$total_reads)
...
```{r}
RRAtab_filt2$richness <- specnumber(RRAtab_filt2[,2:176])
...
# save a copy
#write.csv(RRAtab_filt2, "RRAtab_filt2.csv", row.names=F)
...
RRA_rare <- read.csv("RRAtab_filt2.csv")
#head(RRA_rare)
...
hr = ggplot(MP2[-which(is.na(MP2$MSW93_Species)),], aes(x=log(HomeRange_km2), y=richness))+
  ...
  annotate("text",x=-2.5, y=5.5,label=paste("R^2==0.27"),parse=T, size=5)+
  annotate("text",x=-2.5, y=5, label=paste("p==0.03"),parse=T, size=5)+
  theme_bw(base_size=14)
...
```{r}
specpool(MPdata_0[,15:176],MPdata_0$Species)
...
speculative = specpool(newMP[,15:176],newMP$Species)
...
MOTU = MOTU %>% mutate_at(vars(`Grants gazelle`:Buffalo), funs(ifelse(.>0,1,0)))
MOTU$Hosts = rowSums(MOTU)
MOTU$MOTU = names(RRA2)[15:189]
...
# takes some time
ord = metaMDS(RRA_present[15:189], distance="jaccard",binary=T, k=3, noshare=T, trymax=100, trace=0)
ord$converged
...
MRCdist = vegdist(MPdata_ok[,15:189], distance="jaccard")
...
# all data
ALLdist = vegdist(RRA_present[,15:189], distance="jaccard")
...
levels(plotMDSMRC2$colors)= c("dodgerblue", "magenta3","green3","maroon1", "maroon3", "maroon", "dodgerblue3",  "deepskyblue3", "dodgerblue4","gold3", "cadetblue1", "greenyellow", "green4", "deepskyblue","lightsteelblue3",  "gold", "darkorchid4", "lightskyblue2", "darkorchid1")
...
weirdvec = which((MDSdf$species=="Buffalo" & MDSdf$x1>0)|(MDSdf$species == "Hippo" & MDSdf$x1 >-1) )
weirdvec
