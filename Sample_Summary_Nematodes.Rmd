...
ggplot(samp_summary2, aes(x=reorder(Species,-prevalence), y=pct_diff))+
  geom_col(aes(fill=Species))+
  geom_text(aes(label=difference), nudge_y=1, size=3)+
  guides(fill=F)+
  labs(x="Species",y="Percent Change",title="Difference between prevalence measures (numbers reflect number removed)")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0))+
  scale_fill_sjplot(palette="aqua")
```
