...
# If the maximum is 519 reads in a blank sample then we could eliminate
# write a loop that will do this for every column
allMOTUs=as.data.frame(OBI_reads)
allMOTUs$sample <- COL.all
dim(OBI_reads)

# inefficient coding so this takes a minute
for(i in 1:dim(OBI_reads)[2]){
...
ggplot(allMOTUs, aes(x=rowSums(as.matrix(allMOTUs[,-1702]))))+ #1702 vs 569
  geom_histogram()

### How many samples are now at zero???
length(which(rowSums(allMOTUs[,-1702])==0)) 

rowSums(allMOTUs[allMOTUs$sample=="red",-1702])


#####################
# now calculate the row sums
allMOTUs$total_reads=rowSums(allMOTUs[,-1702])
...
rowSums(allMOTUs[allMOTUs$sample=="red",-(1702:1703)]) # only 4 extraction controls

# ok, now which columns have nothing?
length(which(colSums(allMOTUs[,-(1702:1703)])==0))


row.names(allMOTUs) <- row.names(OBI_reads)
#write.csv(allMOTUs, "allMOTUs_sumatra99_GTmethod2020.csv")
str(allMOTUs)
allMOTUs2 = as.data.frame(allMOTUs)
str(allMOTUs2)

#write.table(as.data.frame(allMOTUs), "allMOTUs_sumatra99_GTmethod2020.csv",sep=",")



########################## This not used for GT method
############################ Johan's stuff here
...
write.table(data.frame(OBI2_motus,tmp), '99_pct_dataset.tab', row.names=F, col.names=T, sep="\t")
